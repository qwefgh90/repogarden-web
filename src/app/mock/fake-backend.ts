            ...
            if (connection.request.url.split('?')[0].match(/repositories\/([^\/]+)\/([^\/]+)/) != null && connection.request.method === RequestMethod.Put) {
                ...
                if (repositoryToFind == undefined) {
                    ...
                    if (bodyObject.hasOwnProperty('activated'))
                        REPOSITORIES[indexToFind] = new Repository(repositoryToFind['name'], repositoryToFind['accessLink'], bodyObject['activated'], repositoryToFind['defaultBranch'], repositoryToFind['branches'], repositoryToFind['cves']);
                    connection.mockRespond(new Response(new ResponseOptions({
                        status: 200
                    })));
                }
